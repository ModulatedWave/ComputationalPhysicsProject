...
# Define the model parameters: The transverse field Ising model
...
for n in steps:
    ...
    for i in tqdm(range(L)):
        ...
        eng = tebd.TEBDEngine(phi, H, tebd_params)
...
np.savetxt('otoc_tfi_first_serious_run.txt', otoc)
plt.figure(figsize=(8, 6))
plt.imshow(otoc, aspect='auto', origin='lower', extent=[0, L, 0, steps[-1]])
plt.colorbar()
plt.savefig('otoc_tfi_first_serious_runv2.pdf')
